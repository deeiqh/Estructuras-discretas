...
int main()
{
    ...
    Tipo x = 1452562;//x = 768454923;
    Tipo y = 1231292;//y = 542167814;
...
Tipo digitos_base(Tipo num, Tipo arr[])
{
    return num / b_binaria(num, arr, 0, MAX_POTENCIAS+2 -1);
}

Tipo b_binaria(Tipo x, Tipo arr[], Tipo low, Tipo high)
{
    Tipo medio = (high-low)/2 + low;
    if(arr[medio] == x || (low > high)) 
        return arr[low-1]; //medio == low, asi retorna la potencia menor de la base
    else{
        if (arr[medio] < x) 
